...
public class StepImpl extends ModelInstance<Step,Hr> implements Step {
    ...
    public static class CLASS extends ActionHome<Hr> {
        ...
        public void createStep( final int p_Value ) throws XtumlException {
            context().LOG().LogInfo( "Attempting to add a new Step." );
            Step step = context().Step_instances().anyWhere(selected -> ((Step)selected).getSValue() == p_Value);
            if ( step.isEmpty() ) {
    ...
    // selections
    private JobSet R3_Job_set;
    @Override
    public void addR3_Job( Job inst ) {
        R3_Job_set.add(inst);
    }
    @Override
    public void removeR3_Job( Job inst ) {
        R3_Job_set.remove(inst);
    }
    @Override
