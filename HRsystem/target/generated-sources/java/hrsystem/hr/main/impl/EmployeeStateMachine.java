...
public class EmployeeStateMachine extends StateMachine<Employee,Hr> {
    ...
    private void On_Leave_entry_action( final long p_Starting,  final String p_Ending,  final int p_National_ID,  final String p_Name ) throws XtumlException {
    ...
    private void On_Leave_requestLeave_txn_to_On_Leave_action( final long p_Starting,  final String p_Ending,  final int p_National_ID,  final String p_Name ) throws XtumlException {
    }

    private void On_Leave_returnFromLeave_txn_to_Working_action() throws XtumlException {
    }

    private void Recruited_commencedRcvd_txn_to_Working_action() throws XtumlException {
    }

    private void Working_requestLeave_txn_to_On_Leave_action( final long p_Starting,  final String p_Ending,  final int p_National_ID,  final String p_Name ) throws XtumlException {
    }
    ...
    public ITransition[][] getStateEventMatrix() {
        return new ITransition[][] {
            { CANT_HAPPEN,
              CANT_HAPPEN,
              (event) -> {Recruited_commencedRcvd_txn_to_Working_action();Working_entry_action();return Working;},
              CANT_HAPPEN
            },
            { CANT_HAPPEN,
              (event) -> {On_Leave_returnFromLeave_txn_to_Working_action();Working_entry_action();return Working;},
              CANT_HAPPEN,
              (event) -> {On_Leave_requestLeave_txn_to_On_Leave_action((long)event.get(0),  (String)event.get(1),  (int)event.get(2),  (String)event.get(3));On_Leave_entry_action((long)event.get(0),  (String)event.get(1),  (int)event.get(2),  (String)event.get(3));return On_Leave;}
            },
            { CANT_HAPPEN,
              CANT_HAPPEN,
              CANT_HAPPEN,
              (event) -> {Working_requestLeave_txn_to_On_Leave_action((long)event.get(0),  (String)event.get(1),  (int)event.get(2),  (String)event.get(3));On_Leave_entry_action((long)event.get(0),  (String)event.get(1),  (int)event.get(2),  (String)event.get(3));return On_Leave;}
            }
        };
    }
