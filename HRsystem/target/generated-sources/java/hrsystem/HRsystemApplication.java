...
public class HRsystemApplication implements IApplication {

    private IComponent<?>[] components;
    private ApplicationExecutor[] executors;

    public HRsystemApplication() {
    ...
    public void setup( String[] args, ILogger logger ) {
        for ( int i = 0; i < executors.length; i++ ) {
            if ( null != logger ) {
                executors[i] = new ApplicationExecutor( "HRsystemApplicationExecutor" + i, args, logger );
            }
            else {
        ...
        ((UI)components[0]).App_Ops().satisfy(((Hr)components[1]).UI_Leave());
        ((Hr)components[1]).UI_Leave().satisfy(((UI)components[0]).App_Ops());
    }
    ...
    public static void main( String[] args ) {
        IApplication app = new HRsystemApplication();
        app.setup( args, null );
